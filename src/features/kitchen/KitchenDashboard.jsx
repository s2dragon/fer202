...
export default function KitchenDashboard({ onBack }) {
  ...
  const groupedOrders = useMemo(() => {
    ...
    const validItems = orderItems.filter((item) =>
      STATUS_FLOW.includes(item.status),
    );
    ...
    if (filterTableId) {
      result = result.filter(
        (o) => String(o.tableId) === String(filterTableId),
      );
    }
    ...
    if (filterStatus !== "all") {
      if (filterStatus === "pending_or_preparing") {
        result = result.filter((o) =>
          o.items.some(
            (it) => it.status === "pending" || it.status === "preparing",
          ),
        );
      } else {
        result = result.filter((o) =>
  ...
  const changeItemStatus = async (item) => {
    ...
    try {
      ...
      setOrderItems((prev) =>
        prev.map((x) => (String(x.id) === String(item.id) ? patchedItem : x)),
      );
      ...
      if (nextStatus === "served") {
        ...
        if (allServed) {
          ...
          setOrders((prev) =>
            prev.map((o) =>
              String(o.id) === String(patchedOrder.id) ? patchedOrder : o,
            ),
          );
        }
      }
  ...
  return (
    <Container fluid className="py-4" style={{ maxWidth: "1200px" }}>
      ...
      {groupedOrders.length === 0 && (
        <div className="text-muted fst-italic">Không có đơn hàng phù hợp</div>
      )}
        ...
        {groupedOrders.map((order, index) => {
          ...
          return (
            <Accordion.Item
              ...
              key={order.id}
              className="border-0 mb-3 rounded-4 overflow-hidden shadow-sm"
            >
              <Accordion.Header className="order-accordion-header">
                <div className="d-flex justify-content-between align-items-center w-100 pe-3">
                  <div className="d-flex align-items-center gap-3">
                    <Badge bg="danger" className="p-2 fs-6 rounded-3">
                    ...
                    <div>
                      ...
                      <div className="text-muted small mt-1">
                        Thời gian:{" "}
                        <span className="fw-bold text-dark">{timeStr}</span>
                        {order.note && (
                          <span className="ms-2 text-danger fst-italic flex-wrap">
                            - Note: {order.note}
                          </span>
                        )}
                      </div>
              ...
              <Accordion.Body className="bg-light p-3 border-top">
                <Table
                  responsive
                  hover
                  className="mb-0 bg-white rounded-3 overflow-hidden shadow-sm border border-light"
                >
                  <thead className="table-light">
                    <tr>
                      <th className="py-3 px-4 text-muted small text-uppercase">
                        Tên món
                      </th>
                      <th className="py-3 px-4 text-muted small text-uppercase text-center">
                        SL
                      </th>
                      <th className="py-3 px-4 text-muted small text-uppercase text-center">
                        Trạng thái hiện tại
                      </th>
                      <th className="py-3 px-4 text-muted small text-uppercase text-end">
                        Thao tác bếp
                      </th>
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={item.id} className="align-middle">
                          ...
                          <td className="py-3 px-4 text-center">
                            <Badge bg="secondary" className="fs-6 px-2">
                              {item.quantity}
                            </Badge>
                          </td>
                          <td className="py-3 px-4 text-center">
                            <Badge
                              ...
                              className="text-uppercase rounded-pill py-1 px-3"
                            >
                              {item.status}
                            </Badge>
                          </td>
